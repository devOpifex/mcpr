...
scatter_plot <- new_tool(
  ...
  handler = function(input) {
    ...
    # Create the plot using base R (to avoid dependencies)
...
simple_forecast <- new_tool(
  ...
  handler = function(input) {
    ...
    # Create a simple plot of historical + forecast values
    # Note: In a production environment, consider file cleanup strategies
    # that don't risk deleting files before clients can access them
    temp_file <- tempfile(fileext = ".png")
    ...
    all_values <- c(values, forecast_values)
    time_points <- 1:length(all_values)
    historical_points <- 1:length(values)
    forecast_points <- (length(values) + 1):length(all_values)
    
    # Plot historical data
    plot(
      historical_points, values,
      type = "l", col = "black", lwd = 2,
      xlim = c(1, length(all_values)),
      ylim = c(min(all_values) * 0.9, max(all_values) * 1.1),
      main = paste("Time Series Forecast (", method, ")"),
      xlab = "Time Period",
      ylab = "Value"
    )
    
    # Add forecast
    lines(forecast_points, forecast_values, col = "blue", lwd = 2, lty = 2)
